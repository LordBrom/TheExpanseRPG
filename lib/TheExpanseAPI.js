    ...
    if (args.shift() === '!theexpanse' && args.length) {
        ...
        try  {
            ...
            if (subtype === 'damage' && args.length === 5) {
                ...
                if (first_die === second_die || second_die === drama_die || first_die === drama_die) sp = drama_die
                ...
                table_format += "[TRB][TDB colspan='3']Result: " + (total + total_modifiers)
                if (tn !== 'x') table_format += " vs. " + tn
                table_format += "[TDE][TRE]" +
                "[TTE]~C"
